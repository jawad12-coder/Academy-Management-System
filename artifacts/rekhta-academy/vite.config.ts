...
export default defineConfig(async ({ mode }) => {
  // Keep local credentials in the workspace root, outside the public app
  // directory. Vite config runs before normal client env loading, so load it
  // explicitly here.
  ...
  define: {
    ...
    'import.meta.env.VITE_SUPABASE_URL': JSON.stringify(env.VITE_SUPABASE_URL ?? resolveEnvVar('SUPABASE_URL')),
    'import.meta.env.VITE_SUPABASE_ANON_KEY': JSON.stringify(env.VITE_SUPABASE_ANON_KEY ?? resolveEnvVar('SUPABASE_ANON_KEY')),
