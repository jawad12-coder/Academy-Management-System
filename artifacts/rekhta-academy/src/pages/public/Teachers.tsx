...
import sirShoaibPhoto from '@assets/sir_sohaib_1782659947704.jpeg';
import sirJunaidPhoto from '@assets/Sir junaid.jpeg';
import sirMudasirPhoto from '@assets/Sir2_1782659969969.jpeg';
import missShanzilPhoto from '@assets/miss shanzil.jpeg';
import missUzraPhoto from '@assets/miss uzra.jpeg';

const localTeacherPhotos: Record<string, string> = {
  'sir shoaib': sirShoaibPhoto,
  'sir sohaib': sirShoaibPhoto,
  'sir junaid': sirJunaidPhoto,
  'sir mudasir': sirMudasirPhoto,
  'miss shanzil': missShanzilPhoto,
  'miss uzra': missUzraPhoto,
};
...
export function Teachers() {
  ...
  return <div className="w-full bg-background pt-20">
    <div className="bg-primary text-white py-20 text-center"><div className="container mx-auto px-4">
      ...
      <p className="text-lg text-primary-foreground/80 max-w-2xl mx-auto">Meet the dedicated educators helping Rekhta Academy learners shine together.</p>
    </div></div>
    <section className="py-20"><div className="container mx-auto px-4">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{teachers.map((teacher, i) => {
        const photo = teacher.photo_url || localTeacherPhotos[teacher.full_name.trim().toLowerCase()];
        return <motion.div key={teacher.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="bg-card rounded-2xl border border-border overflow-hidden shadow-lg">
          <div className="aspect-[4/5] bg-muted relative overflow-hidden flex items-center justify-center">
            {photo ? <img src={photo} alt={teacher.full_name} className="w-full h-full object-cover object-center" /> : <div className="w-24 h-24 rounded-full bg-primary/10 text-primary flex items-center justify-center font-serif text-4xl font-bold">{teacher.full_name.split(' ').map(part => part[0]).slice(0, 2).join('')}</div>}
